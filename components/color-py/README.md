...
make build  # Output: target/color-py.wasm
```

## Compose

```bash
wasmcp compose server target/color-py.wasm -o server.wasm
```
...
# Stdio
wasmcp compose server target/color-py.wasm -t stdio -o server.wasm
